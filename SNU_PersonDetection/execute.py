...
def do_detect(args, detection_network, img, original_img, labels=None):
    ...
    if labels != []:
        ...
        labels= xywhn2xyxy(labels[0], w=W, h=H, padw=0, padh=0)
        ids = find_gt_ids(det, labels.to(detection_network.device))
        return pred_images, det, ids, labels
    else:
        return pred_images, det, None, None
...
def save_result(args, path, original_img, det, pred_class, names, label_gt):
    annotator = Annotator(original_img.numpy(), line_width=2, font_size=18, pil=True, example=str(names))
    ...
    if args.use_GT_IDs:
        ...
        for box_gt in label_gt:
            if int(box_gt[4]) < 0:
                continue
            label = f'{int(box_gt[4])}'
            annotator.box_label(box_gt[:4].cpu().detach().numpy(), label, color=colors(5, True))

...
from util.util import do_moteval, save_pred, make_video
...
class ReIDPerson:
    ...
    def infer(self, dataloader):
        ...
        gt_txt_path = self.args.gt_txt_path
        result_txt_path = os.path.join(os.path.join(self.args.output_dir), 'result.txt')
        ...
        for idx, data in enumerate(tqdm(dataloader)):
            ...
            detect_preds, det, GT_ids, label = do_detect(self.args, self.detection_network, img_preprocess, original_img, labels)
            ...
            save_pred(path[0], det, pred_class, result_txt_path)
            
            
            # SJ Todo
            if self.args.save_images:
                save_result(self.args, path[0], original_img[0], det, pred_class, self.detection_network.names, label)
        ...
        do_moteval(gt_txt_path, result_txt_path)

        if self.args.video:
            make_video(self.args)
